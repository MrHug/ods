...
\begin{itemize}
  ...
  \item #remove()#: remove the next value, #y#, from the #Queue# and return #y#
\end{itemize}
...
include FIFO, priority, and LIFO (the last is used by #Stacks# as opposed to #Queues#).
...
\index{queue!LIFO}%
\index{stack}%
discipline, illustrated in \figref{stack}.  It is sufficiently different from the FIFO #Queue# that it get its own name:
#Stack#, although some texts refer to it as a #LIFO Queue#. In stacks 
...
the stack and also removed from the top of the stack. Often, when discussing a
...
A #Deque#
\index{deque}%
is a generalization of both the FIFO #Queue# and #Stack# (LIFO #Queue#).
...
\begin{exc}
  ...
  provided by the \javaonly{Java Collections Framework}\cpponly{C++
	Standard Template Library}\pcodeonly{language of their choosing}.
